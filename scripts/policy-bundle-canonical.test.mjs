...
test("uses RFC 8785 bytes for a schema-valid, signed read-only policy bundle", async () => {
  ...
  const invalidManifests = [
    {
      ...manifest,
      tools: manifest.tools.map((tool) => tool.name === "get_levels_list" ? { ...tool, retryPolicy: "never" } : tool),
    },
    {
      ...manifest,
      tools: manifest.tools.map((tool) => tool.name === "get_levels_list" ? { ...tool, automaticRollbackAllowed: true } : tool),
    },
    {
      ...manifest,
      tools: manifest.tools.map((tool) => tool.name === "get_rooms_list" ? { ...tool, name: "query_stored_data" } : tool),
    },
  ];
  for (const invalidManifest of invalidManifests) {
    const invalidPolicy = {
      ...policy,
      catalog: { ...policy.catalog, manifest: invalidManifest },
    };
    assert.throws(
      () => signPolicyBundle({ ...bundle, policy: invalidPolicy, policyHash: policyBundleHash(invalidPolicy) }, privateKey, attestation),
      /closed read-only canary/,
    );
  }
